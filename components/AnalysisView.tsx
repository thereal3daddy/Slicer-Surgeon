...
const StepCard = ({ step, index, diagnosis }: { step: FixStep; index: number; diagnosis: string }) => {
  ...
  const tierStyles = [
    ...
    { 
      ...
      text: 'text-blue-400', 
      num: 'bg-blue-500/10 text-blue-400 border-blue-500/20',
      bg: 'bg-blue-500/5',
    ...
    borderHover: 'group-hover:border-slate-700',
    borderActive: 'border-white',
    text: 'text-slate-400', 
    num: 'bg-slate-800 text-slate-400 border-slate-700',
    bg: 'bg-slate-900/40',
  ...
  return (
    <div 
      ...
      className={`group cursor-pointer border rounded-[2rem] transition-all duration-500 relative overflow-hidden ${
        isOpen 
          ? `bg-slate-900/80 ${tierStyles.borderActive} shadow-[0_0_50px_-12px_rgba(0,0,0,0.5)]` 
          : `bg-slate-950/40 ${tierStyles.border} ${tierStyles.borderHover}`
      }`}
    >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-6">
            ...
            <div>
              ...
              <h4 className="text-2xl md:text-3xl font-black text-white tracking-tight uppercase italic">{step.title}</h4>
            </div>
          </div>
          <div className={`w-10 h-10 flex items-center justify-center rounded-full transition-all duration-500 ${isOpen ? 'rotate-180 bg-white text-black' : 'bg-slate-900 text-slate-700 border border-white/5'}`}>
        ...
        {isOpen && (
          <div className="mt-8 pt-8 border-t border-white/5 animate-in fade-in slide-in-from-top-4 duration-500">
            <div className="max-w-3xl">
              <p className="text-slate-400 text-lg leading-relaxed mb-8 italic">"{step.summary}"</p>
              
              <div className="space-y-3 mb-10">
                {(details || step.guide).map((item, i) => (
                  <div key={i} className={`flex items-start space-x-4 p-5 rounded-2xl border transition-all duration-300 ${details ? 'bg-black/40 border-white/10' : 'bg-transparent border-white/5'}`}>
                    <span className={`flex-shrink-0 w-6 h-6 rounded-lg text-[10px] font-black flex items-center justify-center ${details ? tierStyles.num : 'bg-slate-800 text-slate-500'}`}>
                      {i + 1}
                    </span>
                    <span className={`text-base leading-relaxed ${details ? 'text-white' : 'text-slate-300'}`}>{item}</span>
                  </div>
                ))}
              </div>
  ...
  if (!result.isPrintFailure) {
    return (
      <div className="text-center py-20 space-y-8 animate-in fade-in duration-700 max-w-lg mx-auto">
        ...
        <div>
          <h2 className="text-2xl font-black text-white mb-4 uppercase">Recognition Failed</h2>
          <p className="text-slate-500">{result.rejectionMessage || "Could not identify a clear 3D print failure."}</p>
        </div>
        <button onClick={onReset} className="w-full py-4 bg-slate-900 border border-slate-800 text-white font-black rounded-xl uppercase text-[10px] tracking-widest">Try Different Photo</button>
      </div>
    );
  }
    ...
    <div className="space-y-24 animate-in fade-in duration-1000 max-w-5xl mx-auto pb-24">
      ...
      <section className="text-center space-y-8">
        <div className="max-w-2xl mx-auto relative group">
          <div className="absolute -inset-1 bg-blue-500/10 rounded-[2.5rem] blur-2xl opacity-50"></div>
          <div className="relative aspect-video rounded-[2.5rem] overflow-hidden border-[6px] border-slate-900 shadow-2xl">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-center space-x-6">
             <div className="flex flex-col items-center">
              <span className="text-3xl font-black text-white italic">{Math.floor(result.confidenceScore)}%</span>
              <span className="text-[8px] font-mono uppercase text-slate-600 tracking-widest font-bold">Certainty</span>
            </div>
            <div className="w-px h-8 bg-slate-800" />
            <div className="text-left">
              <div className="text-[10px] font-mono font-bold text-blue-500 uppercase tracking-[0.4em]">Primary Diagnosis</div>
              <h2 className="text-4xl md:text-6xl font-black text-white tracking-tighter uppercase italic leading-none">{result.diagnosis}</h2>
            </div>
          </div>
          <p className="text-lg md:text-xl text-slate-400 max-w-2xl mx-auto leading-relaxed">{result.shortDescription}</p>
        </div>
      </section>
      ...
      <section className="space-y-12 relative px-4 sm:px-0">
        ...
        <div className="absolute -inset-x-12 -inset-y-16 bg-blue-600/5 blur-[120px] rounded-full pointer-events-none animate-pulse" />
        
        <div className="relative flex flex-col items-center space-y-3">
          <div className="h-1 w-12 bg-blue-500 rounded-full mb-2 shadow-[0_0_10px_rgba(59,130,246,0.5)]" />
          <h3 className="text-4xl md:text-5xl font-black text-white uppercase italic tracking-tighter text-center">
            3 Quick Fixes
          </h3>
          <p className="text-[11px] font-mono font-bold text-blue-400 uppercase tracking-[0.6em] text-center">Priority Protocol Sequence</p>
        </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-12 gap-8 pt-12">
        <div className="md:col-span-7 p-10 bg-slate-900/30 border border-slate-900 rounded-[2.5rem] shadow-inner">
          ...
          <ul className="space-y-6">
            {result.visualEvidence.map((e, i) => (
              <li key={i} className="text-base text-slate-400 flex items-start space-x-4">
        ...
        <div className="md:col-span-5 flex flex-col justify-between p-10 bg-slate-900/30 border border-slate-900 rounded-[2.5rem] shadow-inner">
          <div className="space-y-8">
            <div className="space-y-2">
              ...
              <div className="p-4 bg-black/40 rounded-2xl border border-white/5">
                <span className="text-xl font-black text-blue-400 uppercase italic tracking-tight">{result.printerGuess}</span>
              </div>
            </div>
              ...
              <div className="space-y-2">
                {result.searchQueries.map((q, i) => (
                  <a 
                    ...
                    target="_blank" 
                    rel="noopener noreferrer"
                    className="flex items-center justify-between p-4 rounded-xl bg-white/5 text-slate-400 hover:text-white hover:bg-white/10 transition-all border border-white/5 text-xs font-bold group"
                  >
                    <span>{q}</span>
          ...
          <button 
            onClick={onReset} 
            className="mt-12 w-full py-6 bg-white text-black font-black text-xs uppercase tracking-[0.4em] rounded-2xl hover:bg-slate-200 transition-all shadow-2xl hover:-translate-y-1 active:translate-y-0"
          >
            Scan Another Photo
          </button>
        </div>
